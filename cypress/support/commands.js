...
Cypress.Commands.add('openTestPostInspection', (status = 'Sanctionable') => {
  cy.visit('/post-inspection/edit')
  cy.selectTestSettings()

  cy.getTestElement('select_inspection').then((selectInspection) => {
    cy.wait(1000).then(() => {
      if (selectInspection.find(`[data-cy~="open_post_inspection_${status}"]`).length !== 0) {
        cy.getTestElement(`open_post_inspection_${status}`).click()
      } else {
